...
| 键 | 含义 | 默认值 | 说明 |
|----|------|--------|------|
| `api_endpoint` | Supabase / API 端点地址 | `https://api.bk.blksails.cn` | 也可用 `--api-endpoint` 标志覆盖；标志默认值即此值（`cmd/root.go` 的 `StringVar(..., "api-endpoint", "https://api.bk.blksails.cn", ...)`，绑定到 viper key `api_endpoint`）。 |
...
# Supabase / API 端点（默认 https://api.bk.blksails.cn，可用 --api-endpoint 覆盖）
api_endpoint: "https://api.bk.blksails.cn"
